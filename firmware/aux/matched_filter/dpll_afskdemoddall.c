...
int main(void)
{
	...
	// ADC values to bit stream

/*
	FILE *raw = fopen("sampled_data.txt", "r");
	while (!feof(raw))
	{
		fscanf(raw, "%d", &trash);
		len++;
	}
	printf("%d ADC samples found\n", len);
	rewind(raw);
	
	uint16_t ADC_data[len];	//TODO: 10-bit result can be shortened to 8-bit (uint8_t sufficient)
	
	for(int i=0; i<len; i++){
		fscanf(raw, "%d", &ADC_data[i]);
	}
	fclose(raw);
	...
	/*
	const uint16_t matched_filter[4][7] = {
		{},
		{},
		{},
		{}
